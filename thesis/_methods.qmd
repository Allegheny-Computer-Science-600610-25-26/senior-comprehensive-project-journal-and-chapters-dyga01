...
pysealer uses environment variables to store and retrieve necesssary cryptographic keys. what are environment variables. Because pysealer relies on environment variables, much of pysealer' security heavily depends on how well these environment variables are managed. raises the ethical concern of relying on environment variables for security. if there is a problem with an envionment variable mangement system. then pysealer could also be compromised.

Key storage and retrieval with python dotenv
cite python dotenv here
[@python-dotenv]

GitHub Secrets integration quote here
what are remote secrets and how does my tool deal with them
[@something]
